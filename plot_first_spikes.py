...
# Version: 
# Last-Updated: Sat Jul 28 14:28:43 2012 (+0530)
#           By: subha
#     Update #: 111
# URL: 
# Keywords: 
...
def plot_early_spikes(files, celltype, t):
    ...
    for f, data in early.items():
        ...
        for ii in range(len(data)):
            plt.plot(data[ii][1], np.ones(len(data[ii][1])) * (ii+1), 'x', label=data[ii][0])
        ...
        ax.text(0.0, ii+1, 'ss: %s, db: %s, daa: %s, dlts: %s' % (d['SpinyStellate'], d['DeepBasket'], d['DeepAxoaxonic'], d['DeepLTS']))
        plt.show()
